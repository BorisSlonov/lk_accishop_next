...
const formSchema = z.object({
  ...
  reflink: z.string().optional().nullable().transform(value => value?.trim() ?? ''),
  inviter: z.string().optional().nullable().transform(value => value?.trim() ?? ''),
...
export default async function signUpAction(
  ...
  const { username, reflink, inviter, email, password } = validatedFields.data;
